    ...
    def forward(self, x):
        ...
        for i in range(self.n_nodes):
            if self.full_input:
                ...
                Par = torch.diag(torch.tensor(self.graph_adj[:, i], device=x.device, dtype=x.dtype))
...
class NoisyFunction(nn.Module):
    """
    Generic wrapper that makes a deep-copied noisy version of an existing nn.Module.
    The base module is deep-copied so the noisy function starts with the same
    weights as the original but is independent. The forward pass adds elementwise
    Gaussian noise (N(0, noise_scale^2)) to the final outputs.
    """
    def __init__(self, base_module, noise_scale=1.0, noise_mean=0.0, noisy_weight=-1):
        super(NoisyFunction, self).__init__()
        import copy
        # deep copy so parameters are independent from the original
        self.base = copy.deepcopy(base_module)
        self.noise_scale = float(noise_scale)
        self.noise_mean = float(noise_mean)
        # noise = torch.randn_like(self.base.layers[-1][0].weight.data) * float(self.noise_scale) + float(self.noise_mean)
        self.base.layers[-1][0].weight.data *= noisy_weight
        #self.base.layers[-1][0].weight.data = torch.sigmoid(torch.atanh(self.base.layers[-1][0].weight.data))
        # self.base.layers[-1][0].weight.data += noise
        self.base.layers[-1][0].compute_weight(update=True)
    def forward(self, x):
        out = self.base(x)
        return out 
