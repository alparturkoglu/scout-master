    ...
    def __init__(
        ...
        if gen_model_provided:
            ...
            if self.sem_type == 'lin':
                self.gen_model = linearSEM(self.graph, self.abs_weight_low, self.abs_weight_high, noise_scale=self.noise_scale, contractive=contractive, noise_type=self.noise_type)
            elif self.sem_type == 'nnl':
                self.gen_model = nonlinearSEM(self.graph, lip_const=self.lip_constant, noise_scale=self.noise_scale, n_hidden=self.n_hidden, act_fun=self.act_fun, noise_type=self.noise_type)
